...
function clone_ensembl_db {
  ...
  if [ ! -e ${sourcedir}/${dbname}.dbcreated ]; then
      mysql -u anonymous -h ${dbhost} -P ${dbport}\
  ...
  if [ ! -e ${sourcedir}/${dbname}.schemaloaded ]; then
      ...
      mysql -u anonymous -h ${dbhost} -P ${dbport} ${dbname} < ${dbname}.sql &&\
  ...
  if [ ! -e ${sourcedir}/${dbname}.dbloaded ]; then
      are_there_txts=$(ls ${sourcedir}/${dbname}/*.txt* &>/dev/null)
      if [ $? -eq 0 ]; then
        cd ${sourcedir}/${dbname}
        gunzip *.txt.gz
        mysqlimport -u anonymous -h ${dbhost} -P ${dbport} --fields_escaped_by=\\ ${dbname} -L `pwd`/*.txt &&\
